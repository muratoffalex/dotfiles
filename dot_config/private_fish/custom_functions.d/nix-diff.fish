function nix-diff
    set -l old_system (command ls -d /nix/var/nix/profiles/system-*-link | sort -V | tail -n2 | head -n1)
    set -l new_system (command ls -d /nix/var/nix/profiles/system-*-link | sort -V | tail -n1)
    set -l old_home (command ls -d ~/.local/state/nix/profiles/home-manager* | sort -V | tail -n2 | head -n1)
